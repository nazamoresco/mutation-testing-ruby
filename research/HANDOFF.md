...
- Foodsoft es el reemplazo seleccionado: Rails ~> 7.2.2, Ruby 3.4.7, RSpec y CI con MySQL+Redis, sin Vite. Tiene 44 commits `fix:` en 1.000 commits; `research/bug-fix-manifests/foodsoft-initial.csv` registra 8 regiones de fix resueltas y 9 controles no-bug (7 resueltos), con límite de cinco regiones por commit. El siguiente paso es el baseline del padre `a8d5cbc8dbdc`; no ejecutar Mutant antes de dos baselines verdes.
