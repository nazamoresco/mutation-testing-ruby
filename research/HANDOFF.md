...
- El estudio histórico de fixes arrancó con Chatwoot en el padre `d3e4ff2dc1e2` del fix de filtros `f452298`. Su preflight relevante pasó (42 ejemplos, 0 fallas). Un primer baseline completo fue inválido por falta de `pnpm`/Vite (6.769 ejemplos, 47 fallas, 67 pendientes); tras instalar el lockfile, el spec de control pasó (4 ejemplos, 0 fallas) y la siguiente suite completa llegó a 6.769 ejemplos en 15 min 55 s con una única falla de `AgentBuilder`, que pasó aislada. Una segunda suite, desde esquema limpio, está en curso para comprobar flakiness. No ejecutar Mutant hasta obtener dos baselines completos verdes. El detalle está en `research/baselines/chatwoot-d3e4ff2dc1e2-2026-09-14.md`.
