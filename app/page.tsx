...
const slides: Slide[] = [
  ...
  { index: '10', section: 'Optimización', minutes: 2.5, title: 'Mutar menos código y ejecutar menos tests', copy: 'mutant-rspec selecciona los examples relevantes para cada sujeto. En un pull request, --since limita la mutación a sujetos con cambios directos; -j distribuye el trabajo entre workers cuando el entorno está aislado.', code: 'bundle exec mutant run \\\n  --use rspec \\\n  --since origin/main \\\n  --jobs 4 \\\n  "Billing::Price#calculate"\n\n# corrida completa: cada noche', annotation: 'El modo incremental acelera el feedback, pero un cambio en una dependencia puede afectar otro sujeto que no aparece en el diff.', presenter: ['Explicá que test selection evita correr toda la suite para cada mutante.', 'Dejá claro el contrapeso: una corrida completa programada sigue siendo necesaria.'], claims: ['La selección de tests reduce el coste de cada mutante.', '--since apunta a los sujetos tocados directamente por el diff.', 'La concurrencia exige aislar base de datos, archivos, cachés y servicios externos.'] },
  { index: '11', section: 'Mutaciones', minutes: 3, title: 'Dos familias de mutaciones hacen preguntas distintas', copy: 'La simplificación semántica elimina o debilita comportamiento. El orthogonal replacement conserva la forma de la operación, pero cambia una decisión por su alternativa válida.', code: 'simplificación semántica\n  age >= 18  →  nil\n\northogonal replacement\n  orders.any?  →  orders.none?', annotation: 'Si el mutante sobrevive, el test no distingue suficientemente el programa original de una alternativa relevante.', presenter: ['Conectá simplificación semántica con código que podría no aportar comportamiento observable.', 'Usá any? → none? para mostrar una decisión que el test debe especificar con precisión.'], claims: ['La simplificación semántica comprueba si el test necesita el comportamiento que se eliminó.', 'Orthogonal replacement comprueba si el test fija qué resultado de una decisión debe ser verdadero.', 'Un mutante vivo puede pedir un test mejor o revelar código redundante.'] },
  { index: '12', section: 'Límites', minutes: 2.5, title: 'Mutation testing concentra el juicio donde importa', copy: 'Tiene coste, mutantes equivalentes y sensibilidad a flakiness. Funciona mejor junto a revisión humana, tipos, análisis estático y threat modeling.', annotation: 'Una herramienta fuerte no reemplaza el criterio: obliga a aplicarlo en un punto concreto.', presenter: ['Decí explícitamente dónde no ayuda para ganar credibilidad.', 'Cerrá esta parte con la pregunta: ¿qué decisiones no nos podemos permitir errar?'], claims: ['100% mutation score no es el producto.', 'El objetivo es verificar decisiones relevantes, no coleccionar métricas.'] },
  { index: '13', section: 'Cierre', minutes: 3, title: 'Si la IA escribe más código, la escasez no será escribir', copy: 'El valor se mueve hacia herramientas que verifican, restringen y explican software: mutación, complejidad, análisis estático, contraejemplos y observabilidad de decisiones.', annotation: 'Menos skills que producen código sin verificar. Más metacódigo que convierte velocidad en confianza calibrada.', presenter: ['Volvé a la pregunta del inicio: ¿vale la pena? Sí, donde una decisión importa más que el coste de comprobarla.', 'Dejá una pregunta abierta sobre qué herramienta “un nivel arriba” falta en el stack de cada persona.'], claims: ['La IA aumenta el valor de la verificación, no elimina la necesidad de criterio.', 'La próxima capa de herramientas puede ser más interesante que otra capa de generación.'] },
];
    ...
    <main className="ruby-light min-h-screen bg-[#fffaf6] text-[#2a171a] selection:bg-[#9c1f31] selection:text-[#fffaf6]">
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 lg:grid-cols-[210px_minmax(0,1fr)]">
        <nav className="border-b border-white/10 p-4 lg:min-h-[calc(100vh-73px)] lg:border-b-0 lg:border-r">
          ...
          <p className="mt-5 border-t border-white/10 pt-4 text-xs leading-relaxed text-white/40"><span className="block font-mono text-[#ffb654]">40 min 30 s</span>de contenido + 5 min de preguntas</p>
        </nav>
            ...
            {!presenterMode ? (
              ...
              <div className="grid gap-6 lg:grid-cols-[1.12fr_.88fr]">
                <div className="border border-[#ffb654]/60 bg-[#1a1c22] p-6 sm:p-9">
                  ...
                  <p className="mt-4 text-xs font-mono text-white/40">Plan de charla: 40 min 30 s de contenido + 5 min de preguntas</p>
